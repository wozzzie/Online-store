    ...
    paintProduct(product: IData) {
        ...
        <li data-id="${product.id}">
          ...
          <div class="product-list">
            <h3>${ellipsis(product.title, 30)}</h3>
            <div class="stars">${product.rating}</div>
            <span class="price">$${product.price}</span>
            <div class="actions">
              <div class="show">
                <a href="javascript:void(0)" class="show-button">Show more</a>
              </div>
              <div class="add-to-cart">
                <a href="javascript:void(0)" data-btn="cart-button" class="cart-button" title="Add to cart">
    ...
    paintCartItem(item: IData) {
        return `
          ...
          <img src="${item.image}" alt="${item.title}" width="20" height="20">
            ...
            <button data-type="remove" data-id="${item.id}">Remove</button>
          </li>
        `;
    }
